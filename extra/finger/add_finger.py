...
def update_data(token):
	push_config = yaml.safe_load(open("extra/finger/指纹数据.yaml", "r", encoding="utf-8").read())
	for i in push_config:
		...
		response = requests.post("https://127.0.0.1:8000/api/fingerprint/", json=payload, headers=headers,timeout=20, verify=False)
...
def do_login():
	burp0_url = "https://127.0.0.1:8000/api/user/login"
